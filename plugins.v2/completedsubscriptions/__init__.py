...
- 修复：SQLAlchemy Instance not bound to a Session 会话解绑报错
"""
...
class CompletedSubscriptions(_PluginBase):
    ...
    plugin_version = "5.7.6"  # 修复SQLAlchemy会话解绑报错
        ...
        for deleted_item in deletion_history:
            ...
            for transfer in transfers:
                ...
                if not fileitems:
                    continue
                for fi in fileitems:
                    associated_files.append(fi.get("path"))
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._cron:
            return [{"id": f"{self.__class__.__name__}_check", "name": "订阅历史清理",
                     "trigger": CronTrigger.from_crontab(self._cron), "func": self.run_check, "kwargs": {}}]
        # 否则，使用默认的固定时间
        else:
            return [{"id": f"{self.__class__.__name__}_check_default", "name": "订阅历史清理 (默认)",
                     "trigger": "cron", "func": self.run_check, "kwargs": {"hour": 3, "minute": 36}}]
                ...
                {'component': 'VRow', 'content': [
                    {'component': 'VCol', 'props': {'cols': 12, 'md': 6},
                     'content': [{'component': 'VSwitch', 'props': {'model': 'enabled', 'label': '启用插件',
                                                                    'hint': '开启或关闭插件功能', 'persistent-hint': True}}]},
                    {'component': 'VCol', 'props': {'cols': 12, 'md': 6},
                     'content': [{'component': 'VSwitch', 'props': {'model': 'notify', 'label': '发送通知',
                                                                    'hint': '任务执行后发送通知消息', 'persistent-hint': True}}]}
                ]},
                {'component': 'VRow', 'content': [
                    {'component': 'VCol', 'props': {'cols': 12, 'md': 6},
                     'content': [{'component': 'VTextField', 'props': {'model': 'days_limit', 'label': '全局天数限制',
                                                                       'type': 'number', 'hint': '只处理超过指定天数的记录，留空则不执行',
                                                                       'persistent-hint': True}}]},
                    {'component': 'VCol', 'props': {'cols': 12, 'md': 6},
                     'content': [{'component': 'VCronField', 'props': {'model': 'cron', 'label': '执行周期 (CRON)',
                                                                       'hint': '留空则每日凌晨3点36分执行一次',
                                                                       'persistent-hint': True}}]}
                ]},
                {'component': 'VRow', 'content': [
                    {'component': 'VCol', 'props': {'cols': 12},
                     'content': [{'component': 'VTextarea', 'props': {'model': 'users_list', 'label': '用户列表',
                                                                      'rows': 4,
                                                                      'hint': '每行一个用户，支持格式 "用户名" 或 "用户名:天数" 来覆盖全局天数限制。',
                                                                      'persistent-hint': True}}]}
                ]},
                {'component': 'VRow', 'content': [
                    {'component': 'VCol', 'props': {'cols': 12, 'md': 4},
                     'content': [{'component': 'VSwitch', 'props': {'model': 'onlyonce', 'label': '保存后立即运行一次',
                                                                    'hint': '该开关会在执行后自动关闭', 'persistent-hint': True}}]},
                    {'component': 'VCol', 'props': {'cols': 12, 'md': 4},
                     'content': [{'component': 'VSwitch', 'props': {'model': 'confirm_delete', 'label': '确认删除',
                                                                    'color': 'error',
                                                                    'hint': '开启后将真实删除文件和订阅 history，请谨慎操作！',
                                                                    'persistent-hint': True}}]},
                    {'component': 'VCol', 'props': {'cols': 12, 'md': 4},
                     'content': [{'component': 'VSwitch', 'props': {'model': 'transfer_cleanup',
                                                                    'label': '整理记录遗留清理', 'color': 'warning',
                                                                    'hint': '按插件删除历史反查整理记录，用于清理旧版本未删干净的文件，建议临时开启一次。',
                                                                    'persistent-hint': True}}]}
                ]},
                {'component': 'VRow', 'content': [
                    {'component': 'VCol', 'props': {'cols': 12}, 'content': [
                        {'component': 'VAlert', 'props': {'type': 'info', 'variant': 'tonal',
                                                          'text': '此插件会扫描所有订阅 history。只有当“全局天数限制”或用户独立天数，以及“用户列表”都填写时，才会按订阅 history 执行。开启“整理记录遗留清理”后，会额外根据本插件已删除历史反查整理记录，用于清理旧版本未删干净的文件。'}}
                    ]}
                ]}
            ]}
    ...
    def _execute_db_operations(self, db: Session = None):
        ...
        try:
            ...
            # 3. 预加载数据，只存ID、路径，不长期持有ORM对象，规避会话解绑
            process_data = []
            ...
            for item in filtered_history:
                ...
                for transfer in transfers:
                    existing_transfer_ids.add(transfer.id)
                    for fi in self.__get_transfer_fileitems(transfer):
                        file_paths.append(fi.get("path"))
                process_data.append({
                    "history_item": item,
                    "download_ids": [d.id for d in downloads],
                    "transfer_ids": [t.id for t in transfers],
                    "files": file_paths,
                    "transfer_cleanup": False
                })

            # 追加遗留清理数据
            if self._transfer_cleanup:
                cleanup_list = self.__get_transfer_cleanup_results(db, existing_transfer_ids)
                process_data.extend(cleanup_list)

            if not process_data:
                return []

            deleted_items_for_page = []
            # 4. 删除模式：全部通过ID操作，不访问已删除ORM对象
            if self._confirm_delete:
                for data in process_data:
                    history_item = data["history_item"]
                    item_name = self.__get_item_value(history_item, "name", "未知标题")
                    item_user = self.__get_item_value(history_item, "username", "未知用户")
                    item_image = self.__get_item_value(history_item, "poster") or self.__get_item_value(history_item, "backdrop")
                    is_cleanup = data.get("transfer_cleanup", False)
                    file_list = data.get("files", [])
                    transfer_objs = data.get("transfers", [])

                    # 遗留清理分支：直接操作查询出来的transfer对象
                    if is_cleanup and transfer_objs:
                        for transfer in transfer_objs:
                            ...
                            for fi in self.__get_transfer_fileitems(transfer):
                                ...
                                if not fp:
                                    continue
                                try:
                                    self.storage_chain.delete_file(FileItem(**fi))
                                    if transfer.src_fileitem and fp == transfer.src_fileitem.get("path"):
                                        eventmanager.send_event(EventType.DownloadFileDeleted, {"src": transfer.src})
                                except Exception as e:
                                    logger.warning(f"删除遗留文件失败 {fp}：{str(e)}")
                            # 删除整理记录
                            try:
                                self.transfer_history_oper.delete(transfer.id)
                                logger.info(f"已删除整理 history 记录: {transfer.title} (ID: {transfer.id})")
                            except Exception as e:
                                logger.warning(f"删除整理记录ID{transfer.id}失败：{str(e)}")
                    ...
                    for fp in file_list:
                        if not fp:
                            continue
                        try:
                            self.storage_chain.delete_file(FileItem(path=fp))
                        except Exception as e:
                            logger.warning(f"删除文件失败 {fp}：{str(e)}")
                    # 删除下载记录
                    for did in data["download_ids"]:
                        try:
                            self.download_history_oper.delete_history(did)
                            logger.info(f"已删除下载 history 记录 (ID: {did})")
                        except Exception as e:
                            logger.warning(f"删除下载记录ID{did}失败：{str(e)}")
                    # 删除整理记录
                    for tid in data["transfer_ids"]:
                        try:
                            self.transfer_history_oper.delete(tid)
                            logger.info(f"已删除整理 history 记录 (ID: {tid})")
                        except Exception as e:
                            logger.warning(f"删除整理记录ID{tid}失败：{str(e)}")
                    # 删除订阅记录
                    try:
                        SubscribeHistory.delete(db, history_item.id)
                        logger.info(f"已删除订阅 history 记录: {item_name} (ID: {history_item.id})")
                        deleted_items_for_page.append({
                            "title": item_name,
                            "user": item_user,
                            "type": self.__get_item_value(history_item, "type"),
                            "year": self.__get_item_value(history_item, "year"),
                            "season": self.__get_item_value(history_item, "season"),
                            "tmdbid": self.__get_item_value(history_item, "tmdbid"),
                            "doubanid": self.__get_item_value(history_item, "doubanid"),
                            "image": item_image,
                            "delete_time": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        })
                    except Exception as e:
                        logger.warning(f"删除订阅记录ID{history_item.id}失败：{str(e)}")

                # 持久化删除记录
                if deleted_items_for_page:
                    all_del = self.get_data("deletion_history") or []
                    all_del.extend(deleted_items_for_page)
                    self.save_data("deletion_history", all_del[-1000:])

            # 组装返回结果（仅用于日志展示，不做删除操作）
            results = []
            for d in process_data:
                results.append({
                    "history_item": d["history_item"],
                    "downloads": [],
                    "transfers": d.get("transfers", []),
                    "files": d["files"],
                    "transfer_cleanup": d.get("transfer_cleanup", False)
                })
            return results
    ...
    def run_check(self):
        ...
        logger.info(
            f"【{self.plugin_name}】：全局天数限制为 {self._days_limit} 天，用户配置为 {self._users_config}，整理记录遗留清理为 {self._transfer_cleanup}")
        if self._confirm_delete:
