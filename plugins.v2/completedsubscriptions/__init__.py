...
class CompletedSubscriptions(_PluginBase):
    ...
    plugin_version = "5.7.5" # 修复删除时 DetachedInstanceError 报错
    ...
    def __get_transfer_cleanup_results(self, db: Session, existing_transfer_ids):
        ...
        for deleted_item in deletion_history:
            ...
            if delete_time:
                ...
                # 修复：使用解析后的 datetime 对象，而非原始字符串

                query = query.filter(TransferHistory.date <= delete_time)
        ...
        try:
            ...
            if self._confirm_delete:
                ...
                for result in results:
                    ...
                    # 提取所有需要删除的文件信息（转为普通字典，脱离 ORM 对象）

                    file_delete_tasks = []  # [(fileitem_dict, src_path_or_None), ...]

                    transfer_ids_to_delete = []

                    transfer_titles = {}  # id -> title，用于日志

                    download_ids_to_delete = []

                    download_titles = {}  # id -> title，用于日志

                    for transfer in transfers:

                        # 提前取出所有需要的属性，避免删除后再访问 ORM 对象

                        transfer_ids_to_delete.append(transfer.id)

                        transfer_titles[transfer.id] = transfer.title

                        src_path = transfer.src if transfer.src_fileitem else None

                        src_fileitem_path = transfer.src_fileitem.get("path") if isinstance(transfer.src_fileitem, dict) else None
                        ...
                        for fileitem in self.__get_transfer_fileitems(transfer):
                            ...
                            is_src = (src_fileitem_path is not None and file_path == src_fileitem_path)

                            event_src = src_path if is_src else None

                            # 转为普通字典，彻底脱离 ORM

                            file_delete_tasks.append((dict(fileitem), event_src))

                    for download in downloads:

                        download_ids_to_delete.append(download.id)

                        download_titles[download.id] = download.title

                    # 执行文件删除

                    for fileitem_dict, event_src in file_delete_tasks:

                        try:

                            self.storage_chain.delete_file(FileItem(**fileitem_dict))

                            if event_src:

                                eventmanager.send_event(EventType.DownloadFileDeleted, {"src": event_src})

                        except Exception as err:

                            logger.warning(f"删除文件失败: {fileitem_dict.get('path')}，错误: {err}")

                    # 执行整理记录删除（只传 id 和 title，不再访问 ORM 对象）

                    for tid in transfer_ids_to_delete:

                        try:

                            self.transfer_history_oper.delete(tid)

                            logger.info(f"已删除整理 history 记录: {transfer_titles.get(tid, '未知')} (ID: {tid})")

                        except Exception as err:

                            logger.warning(f"删除整理记录失败 (ID: {tid})，错误: {err}")

                    # 执行下载记录删除

                    for did in download_ids_to_delete:

                        try:

                            self.download_history_oper.delete_history(did)

                            logger.info(f"已删除下载 history 记录: {download_titles.get(did, '未知')} (ID: {did})")

                        except Exception as err:

                            logger.warning(f"删除下载记录失败 (ID: {did})，错误: {err}")
