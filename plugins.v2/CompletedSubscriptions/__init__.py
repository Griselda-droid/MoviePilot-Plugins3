...
from app.schemas import NotificationType, PluginConfig
from app.utils.timer import TimerUtils

# 插件主类，类名必须是文件夹名称的驼峰式命名
class CompletedSubscriptions(_PluginBase):
    ...
    def get_service(self) -> List[Dict[str, Any]]:
        ...
        if self._cron:
            return [{
                "id": "CompletedSubscriptions_check",
                "name": "已完成订阅检查任务",
        ...
        # 如果没有定义，则每天凌晨随机时间执行一次
            ...
            return [{
                "id": f"CompletedSubscriptions_check_random",
                "name": "已完成订阅检查任务 (随机)",
                "trigger": "cron",
                "func": self.run_check,
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'model': 'cron',
                                            'label': '执行周期 (CRON表达式)',
                                            'placeholder': '留空则每日凌晨随机执行，例如: 0 3 * * *'
                                        }
                                    }
                                ]
                            }
                                ...
                                'content': [
                                    {
                                        ...
                                        'props': {
                                            'type': 'info',
                                            'variant': 'tonal',
                                            'text': '此插件用于扫描所有已完成下载的订阅，并在日志中输出一个清晰的列表，显示每个媒体是由哪个用户订阅的。'
                                        }
                                    }
                                ]
                             }
