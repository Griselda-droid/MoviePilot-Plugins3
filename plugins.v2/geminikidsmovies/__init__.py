...
class GeminiKidsMovies(_PluginBase):
    ...
    plugin_version = "1.7.5" # 致命修正：修复详情页空白问题
    ...
    def get_page(self) -> List[dict]:
        ...
        # 致命修正：使用 VWindow 和 VPagination 手动实现分页
        items_per_page = 200
        pages = [added_history[i:i + items_per_page] for i in range(0, len(added_history), items_per_page)]
                    ...
                    'component': 'VCard', 'content': [
                        {'component': 'div', 'props': {'class': 'd-flex flex-no-wrap justify-space-between'}, 'content': [
                            {'component': 'div', 'content': [
                                ...
                                {'component': 'VCardSubtitle', 'text': f'年份: {item.get("year", "未知")}'},
                                {'component': 'VCardText', 'text': f'添加于: {item.get("add_time", "未知")}'}
                            ]},
